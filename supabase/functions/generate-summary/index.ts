...
Respond with ONLY the JSON structure specified in the system prompt.`
    ...
    const responseToSend = JSON.stringify(parsedResponse);
    console.log('📤 Sending response:', {
      hasSummaryAPerspective: !!parsedResponse.summary_a_perspective,
      hasSummarySharedNeutral: !!parsedResponse.summary_shared_neutral,
      hasContextData: !!parsedResponse.context_data,
      contextDataHasAPerspective: !!parsedResponse.context_data?.summary_a_perspective,
      contextDataHasSharedNeutral: !!parsedResponse.context_data?.summary_shared_neutral,
      summaryAPerspectiveValue: parsedResponse.summary_a_perspective?.substring(0, 100) || 'MISSING',
      summarySharedNeutralValue: parsedResponse.summary_shared_neutral?.substring(0, 100) || 'MISSING',
      responseLength: responseToSend.length,
      responsePreview: responseToSend.substring(0, 1000),
    });
