/*
  ...
  1. Purpose
    - Validates multiple options in a SINGLE Groq API call (major performance improvement)
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    // ========================================
    // STEP 2: Batch validate all non-duplicate options in SINGLE Groq call
    // ========================================
...
Your answer:`;
      try {
        const batchResponse = await fetch('https://api.groq.com/openai/v1/chat/completions', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
