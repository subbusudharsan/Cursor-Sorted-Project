...
function hasKeywordOverlap(text, keywords) {
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    // ========================================
    const duplicateChecks = options.map((option)=>({
        option,
        isDuplicate: previousOptions.some((prevOpt)=>isSimilar(option, prevOpt))
      }));
    const nonDuplicateOptions = duplicateChecks.filter((check)=>!check.isDuplicate).map((check)=>check.option);
...
${nonDuplicateOptions.map((opt, i)=>`${i + 1}. "${opt}"`).join('\n')}
...
Your answer:`;
      try {
        ...
        if (batchResponse.ok) {
          ...
          if (jsonMatch) {
            ...
            nonDuplicateOptions.forEach((option, index)=>{
            ...
            console.log(`✅ Batch validation completed: ${validationResults.filter((r)=>r.overallPass).length}/${nonDuplicateOptions.length} passed`);
          } else {
            // Fallback: accept all if can't parse
            console.warn('⚠️ Could not parse batch validation response, accepting all');
            nonDuplicateOptions.forEach((option)=>{
              validationResults.push({
                option,
    ...
    duplicateChecks.filter((check)=>check.isDuplicate).forEach((check)=>{
      validationResults.push({
        option: check.option,
    ...
    const passedCount = validationResults.filter((r)=>r.overallPass).length;
