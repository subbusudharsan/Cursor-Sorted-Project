...
STRICT RULES:
- Aim for ${wordLimit} words or fewer (can go up to 18 if sentence feels complete)
...
Format as JSON:
{
  ...
  "reasoning": "How hint shaped these responses"` : ''}
    ...
    const validOptions = options.filter(opt => {
      ...
      if (wordCount > wordLimit + 3) { // allow small buffer
  console.warn(`⚠️ Option too long (${wordCount} words) → reject`);
  return false;
}
if (wordCount > wordLimit && wordCount <= wordLimit + 3) {
  console.log(`ℹ️ Option slightly long (${wordCount} words) but still accepted for meaning`);
}
