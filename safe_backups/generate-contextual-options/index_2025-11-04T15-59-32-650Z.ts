...
7. Use context-appropriate pronouns (you/she/he/they based on subject)
...
- MAXIMUM: 15 words per option (hard limit - options exceeding this will be REJECTED, not truncated)
...
- "she said something that really bothered me at work" (9 words) ✅
...
- "I understand you were upset, but I'd like to hear your side" (13 words) ✅
...
Format as JSON:
{
  ...
  "reasoning": "How hint shaped these responses"` : ''}
    ...
    // Filter out options that exceed word limit - REJECT (don't truncate) to ensure meaningful options
    const validOptions = options.filter(opt => {
      ...
      // Allow up to 15 words (10 base + 4-5 tolerance as requested)
      if (wordCount > 15) {
        console.warn(`⚠️ Option exceeds 15 words (${wordCount}) and will be rejected: "${opt.substring(0, 60)}..."`);
        return false;
      }
      return true;
    });
    ...
    // ✅ Be lenient: Accept at least 1 valid option (minimum for conversation flow)
    if (validOptions.length < 1) {
      // Only throw if we have NO valid options at all
      throw new Error(`No valid options after word limit check: all ${options.length} options exceeded 15-word limit`);
    } else if (validOptions.length < 2) {
      console.warn(`⚠️ Only ${validOptions.length} valid option(s) after word limit check. Using what we have.`);
      workingOptions = validOptions;
      console.log(`⚠️ Using ${validOptions.length} valid option(s) (expected ${expectedCount}, but accepting what we have)`);
    } else {
      // Use available valid options (up to expected count)
      workingOptions = validOptions.slice(0, Math.min(expectedCount, validOptions.length));
      console.log(`✅ Word count validation passed: ${workingOptions.length} options under 15 words (expected ${expectedCount})`);
    }
    ...
    // ✅ Be lenient: Accept at least 1 option after deduplication (minimum for conversation flow)
    if (workingOptions.length < 1) {
      ...
      workingOptions = validOptions.length > 0 ? validOptions : workingOptions;
    }
    
    // ✅ Final safety check: Ensure we have at least 1 option
    ...
    // ✅ Final validation: Ensure we have at least 1 option before saving
    if (!finalOptions || finalOptions.length === 0) {
      throw new Error(`No valid options after processing: all options were filtered out`);
    }
