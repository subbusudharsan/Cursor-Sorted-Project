...
- ${isVeryFirstMessage ? '2' : '1'} option: Brief closure text under 15 words ("glad we talked", "thanks for understanding", "this helps")
- ${isVeryFirstMessage ? '3' : '2'} options: Single warm emoji ONLY (🙂, 🤝, ❤️, 👍, 😊)
...
- ${isVeryFirstMessage ? '2' : '1'} option: Brief text acknowledging progress under 15 words ("glad we talked", "thanks for understanding", "this helps")
- ${isVeryFirstMessage ? '3' : '2'} options: Mix of brief text + emoji (e.g., "thanks! 🙂" or "glad we talked 🤝") OR single emoji (🙂, 🤝, ❤️, 👍)
...
Format as JSON:
{
  ...
  "reasoning": "How hint shaped these responses"` : ''}
    ...
    const validOptions = options.filter(opt => {
      ...
      if (wordCount > 15) {
        console.warn(`⚠️ Option exceeds 15 words (${wordCount}) and will be rejected: "${opt}"`);
        return false;
      }
      return true;
    });

    // If we lost too many options, throw error to trigger regeneration
    if (validOptions.length < Math.max(2, expectedCount - 1)) {
    ...
    options = validOptions.slice(0, expectedCount);
    console.log(`✅ Word count validation passed: ${options.length} options under 15 words`);
