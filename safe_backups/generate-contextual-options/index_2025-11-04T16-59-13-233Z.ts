...
const generatingFor = isRecipientUserA ? 'User A' : 'User B';
    const perspectiveLine = generatingFor === 'User A'
...
try {
  ...
  console.warn("⚠️ Perspective sanitization skipped:", err.message);
    ...
    const systemPrompt = `${perspectiveLine} You are writing what one person would DIRECTLY SAY to another person. You are NOT an AI mediator or counselor - you are generating the exact words User A or User B would speak TO each other in a real conversation.
