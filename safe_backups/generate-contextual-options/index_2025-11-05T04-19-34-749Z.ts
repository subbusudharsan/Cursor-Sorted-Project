...
if (recipientEntity && recipientEntity.entity_name) {
  ...
  tagContext += `- Never say their name or refer to them as "she/he" — always use "you/your".\n`;
    ...
    const formattedHistory = safeConversationHistory.map((msg) => {
...
- ✅ CRITICAL: User B is RESPONDING to User A's LATEST MESSAGE: "${currentMessage}"
- User B is NOT continuing User A's original issue - they are RESPONDING to what User A just said
- User B's options should be DIRECT RESPONSES to: "${currentMessage}"
...
- User A just sent you this message: "${currentMessage}"
...
${isRecipientUserB ? `
3. ✅ CRITICAL: Respond DIRECTLY to User A's latest message: "${currentMessage}"
...
` : `
3. Respond to: "${currentMessage}" - USE WORDS/PHRASES from their message to show you heard them
`}
