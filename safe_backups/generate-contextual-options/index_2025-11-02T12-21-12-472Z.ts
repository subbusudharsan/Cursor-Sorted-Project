...
// Build dynamic pronoun tone mapping string
const pronounToneContext = generatingFor === 'User A'
  ? `
🗣️ PRONOUN TONE RULES (User A speaking to User B):
- ${userA?.entity_name || 'User A'} (speaker):
  • Subject → I
  • Object → me
  • Possessive Determiner → my
  • Possessive Pronoun → mine
  • Reflexive → myself
- ${userB?.entity_name || 'User B'} (listener):
  • Subject/Object → you
  • Possessive Determiner → your
  • Possessive Pronoun → yours
  • Reflexive → yourself / yourselves
- Others (third parties): use preferred_pronouns from entity_registry (he/she/they)
`
  : `
🗣️ PRONOUN TONE RULES (User B speaking to User A):
- ${userB?.entity_name || 'User B'} (speaker):
  • Subject → I
  • Object → me
  • Possessive Determiner → my
  • Possessive Pronoun → mine
  • Reflexive → myself
- ${userA?.entity_name || 'User A'} (listener):
  • Subject/Object → you
  • Possessive Determiner → your
  • Possessive Pronoun → yours
  • Reflexive → yourself / yourselves
...
Format as JSON:
{
  ...
  "reasoning": "How hint shaped these responses"` : ''}
    ...
    // ✅ Apply pronoun normalization using existing pronounResolution helper
import { normalizePronounReferences } from '@/lib/pronounResolution';

// Fetch contact names (to resolve “you” vs “Sara”)
...
const currentUser = { name: 'You' }; // or fetch from auth context if stored
const targetContactName = contactData?.display_name || 'Contact';

// Normalize each option’s pronoun references
const normalizedOptions = finalOptions.map(opt =>
  normalizePronounReferences(opt, currentUser.name, targetContactName)
);
