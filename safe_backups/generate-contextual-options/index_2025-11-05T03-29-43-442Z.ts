...
// 🧹 FIXED PERSPECTIVE SANITIZATION — ensures names like @Aradhya always become "you"
try {
  ...
  if (recipientEntity?.entity_name) {
    ...
    fields.forEach(fieldName => {
      let val = eval(fieldName);
      if (typeof val === "string") {
        val = val
          .replace(nameRegex, "you")
          .replace(atTagRegex, "you")
          .replace(/\b(she|her|he|him)\b/gi, "you");
        eval(`${fieldName} = val`);
      }
    });

    console.log(`🧭 Sanitized pronouns for listener (${listenerRole}) → replaced "${recipientEntity.entity_name}" with "you"`);
  }
} catch (err) {
  console.warn("⚠️ Pronoun sanitization skipped:", err.message);
}
  ...
  console.warn("⚠️ Perspective sanitization skipped:", err.message);
    ...
    // 🧭 Perspective Flip for User B
if (isRecipientUserB) {
  // User B is responding → rewrite all @contact tags to "you"
  const contactEntity = entities?.find(e => e.role_in_conversation === "User B");
  const contactName = contactEntity?.entity_name || "";

  const fieldsToFlip = [summary, thoughts, originalIssueSummary, recipientSummary, summaryB, thoughtsB];

  fieldsToFlip.forEach((text, i) => {
    if (typeof text === "string" && text.length > 0) {
      fieldsToFlip[i] = text
        .replace(new RegExp(`@${contactName}`, "gi"), "you")
        .replace(/\b(I|me|my|mine)\b/gi, (m) => m.toLowerCase() === "i" ? "you" : "your");
    }
  });
}
...
Recipient's context:
- Summary (with @# tags): ${summary || recipientSummary || 'No summary provided'}
  ⚠️ IMPORTANT: This summary contains @ and # tags. Use these tags to determine correct pronouns:
  - @ tags = people in the conversation (address with "you" if they're the listener)
  - # tags = third parties being discussed (use their names or pronouns from entity_registry)
...
`}
` : ''}`;
// 🧩 Optional Safety: Replace any leftover @contact mentions with "you"
const cleanSystemPrompt = systemPrompt.replace(/@\w+/g, "you");
