...
// ✅ 1️⃣ PERSPECTIVE CLEANUP - Replace @<listener name> with "you" based on who's speaking
...
// Helper function to clean text by replacing @<listener name> with "you"
const cleanPerspective = (text: string | undefined): string => {
  ...
  if (isRecipientUserA) {
    // Generating for User A → replace @<User B name> with "you"
    if (userBName) {
      // Replace @<User B name> with "you"
      const userBAtTag = new RegExp(`@${userBName}\\b`, 'gi');
      cleaned = cleaned.replace(userBAtTag, 'you');
      // Also replace possessive forms
      cleaned = cleaned.replace(new RegExp(`@${userBName}'s\\b`, 'gi'), "your");
      console.log(`   ✅ Replaced @${userBName} with "you" for User A`);
    }
  } else {
    // Generating for User B → replace @<User A name> with "you"
    if (userAName) {
      // Replace @<User A name> with "you"
      const userAAtTag = new RegExp(`@${userAName}\\b`, 'gi');
      cleaned = cleaned.replace(userAAtTag, 'you');
      // Also replace possessive forms
      cleaned = cleaned.replace(new RegExp(`@${userAName}'s\\b`, 'gi'), "your");
      console.log(`   ✅ Replaced @${userAName} with "you" for User B`);
    }
  }
...
// ✅ Clean all text fields that might contain @<listener> references
...
// ✅ ALSO clean conversation history and current message
...
// 🧩 FIX: Ensure “@UserB” (contact) in summary is also replaced with “you”
const cleanSummaryFinal = cleanPerspective(cleanSummary);
const cleanThoughtsFinal = cleanPerspective(cleanThoughts);
const cleanOriginalIssueSummaryFinal = cleanPerspective(cleanOriginalIssueSummary);
const cleanRecipientSummaryFinal = cleanPerspective(cleanRecipientSummary);
const cleanSummaryBFinal = cleanPerspective(cleanSummaryB);
const cleanThoughtsBFinal = cleanPerspective(cleanThoughtsB);
...
// 🧹 Clean tags inside each message content as well
const cleanHistoryMessage = (text: string) =>
  text.replace(/@\w+’s/g, 'your')
      .replace(/@\w+'s/g, 'your')
      .replace(/@\w+\b/g, 'you');
  ...
  const rawContent = typeof msg === 'object' ? msg.content : String(msg);
  const content = cleanHistoryMessage(rawContent); // ✅ clean here too
  return `${senderId === recipientId ? 'You' : 'Contact'}: ${content}`;
}).join('\n');



    // ✅ Perspective cleanup already done above (lines 176-230) - using cleanSummary, cleanThoughts, etc.
    // All text fields have been cleaned: @<listener name> → "you" based on who's speaking
...
CRITICAL PRONOUN RULES - STRICTLY ENFORCE BASED ON SUMMARY WITH @# TAGS:
${isRecipientUserA ? `
...
- Third parties (#tagged in summary) = Use their names with # prefix OR their pronouns from entity_registry
  Example: "#Sarah didn't invite me" or "she didn't invite me" (if context is clear from summary)
- Keep ALL @ and # tags visible from summary when referring to entities
- The summary contains @# tags - use them to determine correct pronouns
` : `
...
- Third parties (#tagged in summary) = Use their names with # prefix OR their pronouns from entity_registry
  Example: "#Sarah told me about it" or "she mentioned it" (if context is clear from summary)
- Keep ALL @ and # tags visible from summary when referring to entities
- The summary contains @# tags - use them to determine correct pronouns
`}
...
- User A initiated with their concern: "${cleanOriginalIssueSummaryFinal || cleanSummaryFinal}"
...
- When mentioning third parties from summary (with # tags), use their names or pronouns from entity_registry
- Example: "I felt hurt when you ignored me. #Sarah told me about the party." (User A speaking)
` : ''}
${isRecipientUserB ? `
...
- When mentioning third parties from summary (with # tags), use their names or pronouns from entity_registry
- Example: "I understand you felt hurt. I didn't mean to ignore you. #Sarah might have misunderstood." (User B responding)
...
- Original issue (User A): ${cleanOriginalIssueSummaryFinal || cleanSummaryFinal || 'Not specified'}
...
Recipient's context:
- Summary (cleaned, with @# tags): ${cleanSummaryFinal || cleanRecipientSummaryFinal || 'No summary provided'}

  ⚠️ IMPORTANT: This summary contains @ and # tags. Use these tags to determine correct pronouns:
  - @ tags = people in the conversation (listener is addressed with "you", speaker uses "I")
  - # tags = third parties being discussed (use their names with # prefix OR pronouns from entity_registry)
  - The summary has been cleaned: @<listener name> has been replaced with "you" for proper perspective
- Thoughts (cleaned): ${cleanThoughtsFinal || cleanThoughtsBFinal || 'No thoughts provided'}
...
- If hint is about third parties (#tagged in summary), User B tells User A ABOUT them using their names with # prefix OR pronouns from entity_registry (e.g., "#Sarah made me feel excluded", "she mentioned it to me")
...
- Keep ALL @ and # tags visible from summary when referring to entities
- The summary contains @# tags - use them to determine correct pronouns for third parties
...
- The hint context and summary @# tags persist throughout the ENTIRE conversation
` : ''}
...
`}
` : ''}`;
// ✅ System prompt already uses cleaned context - no need for additional cleanup
...
- USE SPECIFIC WORDS from the issue context: ${cleanOriginalIssueSummaryFinal || cleanSummaryFinal}
...
7. ✅ STRICT PRONOUN RULES (based on summary with @# tags):
   ${isRecipientUserA ? `
   - Speaker (User A) = "I / me / my / mine / myself"
   - Listener (User B) = "you / your / yours / yourself" (NEVER use User B's name)
   - Third parties (#tagged) = Use #Name or their pronouns from entity_registry
   ` : `
...
- "#Sarah said something that really bothered me at work" (9 words) ✅
...
Note: User A uses "I" for themselves, "you" for User B, and #Name or pronouns for third parties
` : `
...
Note: User B uses "I" for themselves, "you" for User A, and #Name or pronouns for third parties
`}
...
console.log({
  cleanCurrentMessage,
  cleanConversationHistorySample: cleanConversationHistory.slice(-3),
  cleanSummary,
  cleanThoughts,
});
