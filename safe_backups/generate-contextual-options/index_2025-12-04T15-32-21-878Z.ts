/* 
  ...
  1. Purpose
    ...
    - Uses Groq Llama AI to create empathetic, relevant suggestions
...
// NOTE: This project uses Groq Llama 3.1 8B Instant for cost-effective AI responses.
// The model can be upgraded to Llama 3.1 70B or other models in the future if needed.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ✅ TOKEN OPTIMIZATION: Check if pregenerated turns exist before generating options
    // This prevents unnecessary token usage when pregenerated turns are available
    try {
      const { data: existingPregen, error: pregenCheckError } = await supabase
        .from('pregenerated_turns')
        .select('id, options, turn_number, recipient_id, used')
        .eq('chat_id', chatId)
      ...
      if (!pregenCheckError && existingPregen && existingPregen.options && Array.isArray(existingPregen.options) && existingPregen.options.length > 0) {
        console.log("⚠️ Pregenerated turns exist for this recipient - skipping generate-contextual-options to save tokens");
        console.log("   Pregenerated turn found:", {
          turn_number: existingPregen.turn_number,
          options_count: existingPregen.options.length,
          recipient_id: existingPregen.recipient_id
        });
        return new Response(JSON.stringify({
          success: false,
          error: "Pregenerated turns exist",
          message: "Pregenerated turns are available for this recipient. Use those instead.",
          should_use_pregenerated: true,
          pregenerated_turn_number: existingPregen.turn_number
        }), {
          status: 200,  // 200 because this is expected behavior, not an error
          headers: { ...corsHeaders, "Content-Type": "application/json" }
        });
      }
    } catch (pregenCheckErr) {
      // Non-critical error - log and continue with generation
      console.warn("⚠️ Failed to check pregenerated turns (non-critical, continuing):", pregenCheckErr);
    }
    ...
    const shouldUseHint = isRecipientUserB && hintFromBWithFallback && hintFromBWithFallback.trim().length > 0;
    
    // ✅ DEBUG: Log hint status for troubleshooting
    console.log('🔍 HINT DEBUG:', {
      hintFromB_param: hintFromB || 'NOT PROVIDED',
      hint_from_b_param: hint_from_b || 'NOT PROVIDED',
      contextData_hint_from_b: contextData.hint_from_b || 'NOT IN DB',
      contextData_hintFromB: contextData.hintFromB || 'NOT IN DB',
      hintFromBWithFallback: hintFromBWithFallback || 'EMPTY',
      hintFromBWithFallback_length: hintFromBWithFallback.length,
      isRecipientUserB,
      shouldUseHint,
      recipientId,
      chatData_user_id: chatData?.user_id,
      chatData_contact_id: chatData?.contact_id,
      hintPreview: hintFromBWithFallback.substring(0, 100)
    });
...
if (recipientEntity && recipientEntity.entity_name) {
  ...
  tagContext += `- Always address the listener as "you/your" — never use their real name or refer to them as "she/he".\n`;
    ...
    console.log('  hintFromBWithFallback:', hintFromBWithFallback ? `${hintFromBWithFallback.substring(0, 50)}...` : 'EMPTY');
    console.log('  conversationHistory.length:', safeConversationHistory.length);
    
    // ✅ DEBUG: Log if hint will be used in prompt
    if (shouldUseHint && isRecipientUserB) {
      console.log('✅ HINT WILL BE USED IN PROMPT:', {
        hintLength: hintFromBWithFallback.length,
        hintPreview: hintFromBWithFallback.substring(0, 100),
        isRecipientUserB,
        recipientId
      });
    } else if (isRecipientUserB && !shouldUseHint) {
      console.warn('⚠️ HINT NOT BEING USED FOR USER B:', {
        isRecipientUserB,
        hintFromBWithFallback: hintFromBWithFallback || 'EMPTY',
        hintLength: hintFromBWithFallback.length
      });
...
// ✅ DETECT: Is the latest message casual/social (greeting, check-in, pleasantry) without issue content?
const isCasualOrSocialMessage = (() => {
  const lower = cleanCurrentMessage.toLowerCase();
  
  // Greeting patterns
  const greetingPatterns = [
    /^(hi|hey|hello|hi there|hey there)/i,
    /hope you.*doing/i,
    /how are you/i,
    /how.*going/i,
    /good (morning|afternoon|evening)/i,
    /nice to (hear|see|talk)/i,
    /great to (hear|see)/i,
    /^😊|^🙂|^😁|^👋/,
  ];
  
  // Casual/social patterns
  const casualPatterns = [
    /^just (checking|wanted|thinking|saying)/i,
    /^wanted to (say|tell|share|check)/i,
    /hope.*good|hope.*well/i,
    /have a (nice|good|great) (day|weekend|week)/i,
    /thanks (for|a lot|so much)/i,
    /thank you (for|so much)/i,
    /appreciate (it|you|that)/i,
    /take care/i,
    /talk (soon|later)/i,
  ];
  
  const isGreeting = greetingPatterns.some(pattern => pattern.test(lower));
  const isCasual = casualPatterns.some(pattern => pattern.test(lower));
  
  // Check if message contains actual issue/explanation content
  const hasIssueContent = /\b(felt|hurt|upset|bothered|problem|issue|wrong|angry|sad|confused|worried|disappointed|frustrated|ignored|left out)\b/i.test(lower);
  const hasSpecificEvent = /\b(when|didn't|wasn't|because|after|before|since|while|happened|occurred)\b/i.test(lower);
  const hasExplanation = /\b(reason|why|because|due to|the thing is|what happened was)\b/i.test(lower);
  
  // If it's a greeting/casual message AND has no issue/explanation content, treat as casual
...
// ✅ OPTIMIZED: Smart history selection - last 3 messages for context (reduced from 5 to save tokens)
const totalHistoryLength = cleanConversationHistory.length;
const contextMessages = totalHistoryLength <= 3 
  ? cleanConversationHistory  // If 3 or fewer, use all
  : cleanConversationHistory.slice(-3); // Otherwise, last 3 for context (reduced to save tokens)
...
const lastTwoMessages = cleanConversationHistory.slice(-2).map((msg) => {
  ...
  return content;
    ...
    // Groq receives fully cleaned text - no @ tags should be visible
...
TAG SYSTEM RULES (AFTER PERSPECTIVE CLEANING):
...
${isVeryFirstMessage ? `
VERY FIRST MESSAGE - NATURAL HUMAN OPENING WITH GREETING:
...
CRITICAL: Generate 5 DISTINCT options that follow NATURAL HUMAN CONVERSATION PATTERNS:

1. INCLUDE CASUAL CHECK-IN GREETINGS (like humans do in real life):
   ...
   - "Hi, hope you're doing well. Wanted to bring something up"
   - "Hey, how's everything? Can we talk about what happened?"

2. SITUATION-AWARE OPENINGS (based on context):
   - After a fight: "Hey, I know things got tense earlier, but I wanted to talk about what happened"
   - Happy/positive: "Hi! I've been wanting to talk to you about something..."
   - Embarrassed/awkward: "Hey, this is a bit awkward, but can we talk about something?"
   - Long time no talk: "Hey! It's been a while - hope you're doing well. Can we talk about something?"

3. NATURAL TRANSITION PATTERNS:
   - Greeting → Issue: "Hey, how are you? Can we talk about what happened at the party?"
   - Check-in → Purpose: "Hi, how's it going? There's something I wanted to discuss"
   - Warm → Topic: "Hey there, how have you been? Got something on my mind"

Timing Context: ${conversationTimingContext}

${conversationTimingContext === 'recent_argument' ? `
RECENT ARGUMENT - Acknowledgment + Direct resolution:
Generate 5 options that:
1. Acknowledge the situation: "Hey, I know things got tense earlier..." / "Hi, I'm sorry about how that went..."
2. Then mention wanting to resolve: "...but I'd like to talk about what happened"
Examples:
- "Hey, I know things got tense earlier, but can we talk about what happened?"
- "Hi, I'm sorry about how that went. Can we figure this out?"
- "Hey, that didn't go well. I'd like to clear the air about what happened"
- "Hi, I know we both got upset. Can we talk about what happened?"
- "Hey, I feel bad about how that went. Can we discuss what happened?"
` : conversationTimingContext === 'long_gap' ? `
LONG GAP - Warm reconnection + casual check-in + gentle issue mention:
Generate 5 options that:
1. Start with warm greeting/check-in: "Hey! It's been a while..." / "Hi! Hope you're doing well..." / "Hey, how are you? It's been so long..."
2. Include casual check-in: "...how are you?" / "...how's everything going?" / "...hope you're doing well"
3. Then gently introduce the topic: "...and I wanted to talk about something" / "...can we chat about something?"
Examples:
- "Hey! It's been a while - how are you? Can we talk about something?"
- "Hi! Hope you're doing well. Been thinking about you and something I wanted to discuss"
- "Hey, how's it going? It's been so long. Got something on my mind I'd like to chat about"
- "Hi, how have you been? Miss chatting with you. Can we talk about something?"
- "Hey there, hope everything's good with you. Wanted to bring something up"
` : conversationTimingContext === 'same_day' ? `
SAME DAY - Friendly check-in + issue mention:
Generate 5 options that:
1. Include casual check-in: "Hey, how's your day?" / "Hi, how are you?" / "Hey, how's it going?"
2. Then mention purpose: "...got something I wanted to talk about" / "...can we chat about something?"
Examples:
- "Hey, how's your day? Got something I wanted to talk about"
- "Hi, how are you? Something on my mind I'd like to discuss"
- "Hey, how's it going? Need to chat about something"
- "Hi, hope you're good. Wanted to bring something up"
- "Hey there, how's everything? Can we talk about what happened?"
` : `
NORMAL TIMING - Natural greeting + check-in + issue transition:
Generate 5 options with varied styles (mix of casual greetings and situation-aware):
1. Casual check-in + purpose: "Hey, how are you? Can we talk about something?"
2. Warm greeting + topic: "Hi, hope you're doing well. Got something I wanted to discuss"
3. Friendly check-in + issue: "Hey there, how's everything going? Need to chat about something"
4. Simple greeting + purpose: "Hi, how have you been? Wanted to bring something up"
5. Natural opening + topic: "Hey, got a sec? Something I'd like to talk about with you"
Examples:
...
- "Hey there, how's everything going? Got something on my mind"
- "Hi, how have you been? There's something I'd like to discuss"
- "Hey, got a sec? Can we chat about something that's been bothering me?"
`}

Relationship tone (${contactCategory}):
- Friends: casual, use natural slang if appropriate, include "hey how are you" / "how's it going"
- Family: warm but respectful, include "hi, how are you" / "hope you're doing well"
- Coworkers: professional but friendly, include "hi, how's everything?" / "hope you're well"
- General: balanced and respectful, include casual check-ins naturally

CRITICAL: Generate 5 TRULY DISTINCT options that:
- INCLUDE casual check-in greetings naturally (like "hey how are you", "how's it going", "hope you're doing well")
- THEN transition to the issue/topic from summary (in different ways - direct mention, subtle hint, or implied purpose)
- Vary in style: some start with casual greeting, some are more direct, some are situation-aware
- Vary in directness (subtle hint vs explicit mention)
- Vary in formality (casual vs respectful)
- Vary in emotional tone (calm vs worried vs warm)
...
` : isRecipientUserB ? `
${isCasualOrSocialMessage ? `
✅ USER B - CASUAL/SOCIAL RESPONSE MODE:
- User A sent a casual/social message: "${cleanCurrentMessage}"
- This is a greeting, check-in, or pleasantry - NOT an issue or explanation
- Respond naturally and appropriately to match their tone
- ❌ DO NOT use issue-focused acknowledgment phrases:
  * "That makes sense" (doesn't fit casual messages)
  * "I understand" (too formal for casual)
  * "I hear you" (doesn't fit greetings)
  * "I get it" (doesn't fit casual check-ins)
- ✅ DO respond with appropriate casual/social responses:
  * For greetings: "Hi! I'm doing well, thanks" / "Hey! Good to hear from you" / "Hi there! I'm good, hope you're doing well"
  * For check-ins: "I'm good, thanks for asking" / "Doing well, hope you are too" / "All good here, thanks"
  * For pleasantries: "Thanks! You too" / "Appreciate it" / "Same to you"
  * Match their energy: if they used emojis, you can too
- Keep it warm, friendly, and natural - like normal conversation
...
${canUserBAskToShareAfterHint ? `
✅ USER B - AFTER HINT: Can ask "Can I share something from my side now?"
- After User A says "Yes, please do", User B MUST explain directly using hint details
- ❌ DO NOT generate vague options: "I would like to say", "I thought I can share"
- ✅ DO generate actual explanation from hint context
` : ''}` : ''}
...
console.log("🧹 ============ CLEANED DATA CHECK BEFORE GROQ ============");
...
// ✅ FIX: Add debug log showing recipientSummary before Groq API call
...
console.log(`   Has content: ${cleanRecipientSummary.length > 0 ? 'YES' : 'NO ❌'}`);
    ...
    const callGroqAPI = async (payload: any, maxRetries = 3): Promise<Response> => {
        ...
        try {
          ...
          if (response.status === 429) {
            ...
            if (attempt < maxRetries - 1) {
              ...
              console.error("Groq API error (429 - rate limit):", errorText);
              throw new Error(`Groq API rate limit exceeded after ${maxRetries} attempts`);
            }
          }

          // ✅ FIX: Handle payload too large error (413) - don't retry, fail immediately
          if (response.status === 413) {
            const errorText = await response.text();
            console.error("Groq API error (413 - payload too large):", errorText);
            throw new Error(`Request too large. Please reduce conversation history or summary length.`);
          }
    ...
    const fixPronounMistakes = (option: string): string => {
      ...
      if (isRecipientUserA || isRecipientUserB) {
        // ✅ FIX: Smart pronoun replacement - only replace when clearly referring to listener, not third parties
        possessiveNouns.forEach(noun => {
          // Pattern: "her noun", "his noun", "their noun" → "your noun"
          // BUT: Don't replace if "her/his/their" refers to a third party mentioned nearby
          fixed = fixed.replace(new RegExp(`\\b(her|his|their)\\s+${noun}\\b`, 'gi'), (match, pronoun, offset, source) => {
            ...
            // Check for third-party indicators nearby
            // Pattern: "to her", "about her", "with her", "from her", "talking to her", "congratulating her"
            ...
            // Check if there's a third-party name nearby (capitalized name before the pronoun)
            ...
            // Only replace if NO third-party reference nearby
            // This prevents: "talking to her about her achievements" → "talking to her about your achievements" (wrong!)
    ...
    if (isRecipientUserB && !isVeryFirstMessage) {
      ...
      const filteredOptions = enhancedOptions.filter(opt => {
        ...
        if (isRecipientUserA) {
          // User A shouldn't say things that sound like User B's perspective
          // Example: "I want to help you understand what happened when I was talking to her"
          const userBPerspectivePatterns = [
            /i want to help you understand.*when i was (talking|speaking) to (her|him|them)/i,
            /i want to help you understand.*when i/i,
            /what happened when i was (talking|speaking) to (her|him|them)/i,
            /i was (talking|speaking) to (her|him|them) about your/i,
          ];
          if (userBPerspectivePatterns.some(pattern => pattern.test(lower))) {
            console.log(`   🚫 Blocked option with wrong perspective (User A got User B option): "${opt.substring(0, 50)}"`);
            return false;
          }
        }
        
        if (isRecipientUserB) {
          // User B shouldn't say things that sound like User A's issue explanation
          // Example: "I felt hurt when you were congratulating her"
          const userAPerspectivePatterns = [
            /i felt (hurt|ignored|upset|bothered).*when (you|she|he) (was|were) (congratulating|talking to|speaking to)/i,
            /when (you|she|he) (was|were) (congratulating|talking to|speaking to).*i felt/i,
            /i felt (hurt|ignored|upset|bothered).*congratulating (her|him|them)/i,
          ];
          if (userAPerspectivePatterns.some(pattern => pattern.test(lower))) {
            console.log(`   🚫 Blocked option with wrong perspective (User B got User A option): "${opt.substring(0, 50)}"`);
            return false;
          }
        }
        ...
        // ✅ NEW: Block inappropriate issue-focused responses for casual/social messages
        if (isCasualOrSocialMessage) {
          const inappropriateForCasualPatterns = [
            /that makes sense/i,
            /i (understand|get it|hear you)/i,
            /i (see|know|realize) what you (mean|said)/i,
            /i (acknowledge|appreciate) your (perspective|situation)/i,
          ];
          const isInappropriate = inappropriateForCasualPatterns.some(pattern => pattern.test(lower));
          if (isInappropriate) {
            console.log(`   🚫 Blocked inappropriate response for casual message: "${opt.substring(0, 50)}"`);
            return false;
          }
        }
    ...
    const { data: insertData, error: insertError } = await supabase
      ...
      .insert({
        ...
        source: 'generate-contextual-options', // ✅ Track source
        context_data: {
          validated: true,
