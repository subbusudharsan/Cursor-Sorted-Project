...
The listener (${isRecipientUserA ? userBName : userAName}) MUST NEVER be called by their name in the generated options.
- FORBIDDEN: "I felt hurt when ${isRecipientUserA ? userBName : userAName} said..." ❌
- CORRECT: "I felt hurt when you said..." ✅
- FORBIDDEN: "${isRecipientUserA ? userBName : userAName}'s comment made me..." ❌
- CORRECT: "Your comment made me..." ✅
The listener is ALWAYS addressed as "you/your/yours" - NEVER by their name.
...
console.log(`   Has content: ${cleanRecipientSummary.length > 0 ? 'YES' : 'NO ❌'}`);
    ...
    // Apply stripTagSymbols, fixPronounMistakes, AND capitalizeFirstLetter to all options
    const finalOptions = options.slice(0, Math.min(expectedCount, options.length))
      .map(opt => capitalizeFirstLetter(fixPronounMistakes(stripTagSymbols(opt))));
