...
console.log(`   Has content: ${cleanRecipientSummary.length > 0 ? 'YES' : 'NO ❌'}`);
    ...
    const removeListenerName = (option: string): string => {
      ...
      listenerVariants.forEach(variant => {
        ...
        cleaned = cleaned.replace(new RegExp(`\\b${escaped}\\b`, 'gi'), (_, offset, source) =>
          applySentenceCase('you', offset, source)
        );

        if (!hasSpace) {
          cleaned = cleaned.replace(new RegExp(`@${escaped}'s\\b`, 'gi'), (_, __, offset, source) =>
            applySentenceCase('your', offset, source)
          );
          cleaned = cleaned.replace(new RegExp(`@${escaped}'\\b`, 'gi'), (_, __, offset, source) =>
            applySentenceCase('your', offset, source)
          );
          cleaned = cleaned.replace(new RegExp(`@${escaped}\\b`, 'gi'), (_, offset, source) =>
            applySentenceCase('you', offset, source)
          );
        }
      });
