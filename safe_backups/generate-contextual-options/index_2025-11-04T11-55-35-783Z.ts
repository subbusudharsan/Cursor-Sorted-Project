...
if (recipientEntity && recipientEntity.entity_name) {
  ...
  tagContext += `- Never say their name or refer to them as "she/he" — always use "you/your".\n`;
    ...
    console.log('  conversationHistory.length:', Array.isArray(conversationHistory) ? conversationHistory.length : 0);
...
Recent conversation (${safeConversationHistory.length} turns):
...
📈 Conversation progress: Turn ${safeConversationHistory.length}
...
${safeConversationHistory.length <= 2 ? `
...
Format as JSON:
{
  ...
  "reasoning": "How hint shaped these responses"` : ''}
    ...
    // ✅ ENFORCE WORD LIMIT: Validate and reject if exceeds (don't truncate)
    ...
    // Filter out options that exceed word limit - don't truncate, just reject and regenerate if needed
    const validOptions = options.filter(opt => {
      ...
      if (wordCount > wordLimit) {
        console.warn(`⚠️ Option exceeds ${wordLimit} words (${wordCount}): "${opt.substring(0, 50)}..."`);
        return false;
      }
      return true;
    });
