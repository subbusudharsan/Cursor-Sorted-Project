...
const cleanPerspective = (text: string | undefined): string => {
  ...
  if (isRecipientUserA) {
    ...
    if (userBName) {
      ...
      // Replace possessive forms FIRST (more specific pattern)
      ...
      console.log(`   ✅ Replaced @${userBName} with "you/your" for User A`);
    }
  } else {
    // Generating for User B → replace @<User A name> with "you"
    if (userAName) {
      // Escape special regex characters in name
      const escapedName = userAName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      ...
      console.log(`   ✅ Replaced @${userAName} with "you/your" for User B`);
    }
  }
