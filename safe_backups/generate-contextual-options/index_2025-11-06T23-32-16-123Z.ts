...
const cleanPerspective = (text: string | undefined): string => {
  ...
  if (isRecipientUserA) {
    ...
    // Replace all references to User B with "you/your"
    if (userBName) {
      const escapedName = userBName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

      // Replace possessive forms FIRST (both with @ and without @)
      ...
      // Replace base form (both with @ and without @)
      cleaned = cleaned.replace(new RegExp(`@${escapedName}\\b`, 'gi'), 'you');
      cleaned = cleaned.replace(new RegExp(`\\b${escapedName}\\b`, 'gi'), 'you');

      console.log(`   ✅ User A speaking TO User B: Replaced ${userBName} and @${userBName} with "you/your"`);
    }
  } else {
    // Generating for User B → User B is speaking TO User A
    // Replace references to User B (themselves) with "I/me/my"
    // Replace references to User A with "you/your"

    if (userBName) {
      const escapedBName = userBName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

      // When User B's name appears, they refer to themselves as "I/me/my"
      cleaned = cleaned.replace(new RegExp(`@${escapedBName}'s\\b`, 'gi'), "my");
      cleaned = cleaned.replace(new RegExp(`\\b${escapedBName}'s\\b`, 'gi'), "my");
      cleaned = cleaned.replace(new RegExp(`@${escapedBName}\\b`, 'gi'), "I");
      cleaned = cleaned.replace(new RegExp(`\\b${escapedBName}\\b`, 'gi'), "I");

      console.log(`   ✅ User B perspective: Replaced ${userBName} and @${userBName} with "I/me/my"`);
    }

    if (userAName) {
      const escapedAName = userAName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

      // When User A's name appears, User B addresses them as "you/your"
      cleaned = cleaned.replace(new RegExp(`@${escapedAName}'s\\b`, 'gi'), "your");
      cleaned = cleaned.replace(new RegExp(`\\b${escapedAName}'s\\b`, 'gi'), "your");
      cleaned = cleaned.replace(new RegExp(`@${escapedAName}\\b`, 'gi'), "you");
      cleaned = cleaned.replace(new RegExp(`\\b${escapedAName}\\b`, 'gi'), "you");

      console.log(`   ✅ User B speaking TO User A: Replaced ${userAName} and @${userAName} with "you/your"`);
    }
  }
...
if (recipientEntity && recipientEntity.entity_name) {
  ...
  tagContext += `- Always address the listener as "you/your" — never use their real name or refer to them as "she/he".\n`;
    ...
    const naturalClosureDetected = mutualExchange && emotionalClosureScore >= 0.5;
...
- Summary text has been cleaned: @ became "you/your", # symbols removed
...
Resolution is emerging. Mix acknowledgment with gentle closure signals.

Generate ${isVeryFirstMessage ? '5' : '3'} options:
- ${isVeryFirstMessage ? '2-3' : '1-2'} options: Brief warm acknowledgments (8-12 words)
  Examples: "glad we could talk through this"
           "thanks for hearing me out, means a lot"
           "appreciate you taking time to understand"
- ${isVeryFirstMessage ? '2' : '1'} option: Single warm emoji (🙂, 🤝, ❤️, 😊)

TONE GUIDANCE:
- Acknowledge the emotional journey: "this conversation helped"
- Express gratitude naturally: "thanks for being open with me"
- Show positive forward energy: "feel better about us now"
- Match relationship: casual with friends, warmer with family, respectful with work
...
console.log(`   Has content: ${cleanRecipientSummary.length > 0 ? 'YES' : 'NO ❌'}`);
    ...
    const checkForNameLeak = (text: string): boolean => {
      if (!listenerName || !text) return false;
    ...
    const validOptions = options.filter(opt => {
      ...
      if (checkForNameLeak(opt)) {
        console.warn(`❌ NAME LEAK DETECTED: Option contains listener's name "${listenerName}": "${opt.substring(0, 60)}..."`);
    ...
    const removeListenerName = (option: string): string => {
      ...
      if (listenerName) {
        ...
        // Replace possessive forms: "Aradhya's" → "your"
        cleaned = cleaned.replace(new RegExp(`\\b${escapedName}'s\\b`, 'gi'), 'your');
        cleaned = cleaned.replace(new RegExp(`\\b${escapedName}'\\b`, 'gi'), 'your');
        
        // Replace base form in common patterns: "Aradhya said" → "you said", "what Aradhya said" → "what you said"
        // Use word boundaries to avoid partial matches
        cleaned = cleaned.replace(new RegExp(`\\b${escapedName}\\b`, 'gi'), (match, offset, string) => {
          // Check context to determine if it should be "you" or "I"
          // If User A is speaking, replace User B's name with "you"
          // If User B is speaking, replace User A's name with "you" (listener)
          // But if it's User B's own name, it should be "I" (already handled in cleanPerspective, but double-check)
          if (isRecipientUserA) {
            // User A speaking TO User B - replace User B's name with "you"
            return 'you';
          } else {
            // User B speaking TO User A - replace User A's name with "you"
            return 'you';
          }
        });
      }
    ...
    // Apply removeListenerName, stripTagSymbols, fixPronounMistakes, AND capitalizeFirstLetter to all options
    // Order matters: remove names first, then fix pronouns, then capitalize
    ...
    if (!finalOptionsToUse || finalOptionsToUse.length === 0) {
      // Last resort: use original options with tag cleanup
      if (options && options.length > 0) {
        console.warn("⚠️ Using original options as absolute fallback for finalOptions");
        finalOptionsToUse = options.slice(0, 3).map(opt => capitalizeFirstLetter(stripTagSymbols(String(opt))));
      } else if (optionsData.options && optionsData.options.length > 0) {
        console.warn("⚠️ Using optionsData.options as last resort");
        finalOptionsToUse = optionsData.options.slice(0, 3).map(opt => capitalizeFirstLetter(stripTagSymbols(String(opt))));
      } else {
    ...
    // ✅ Ensure we have valid options array and apply final tag cleanup
    const normalizedOptions = Array.isArray(finalOptionsToUse) && finalOptionsToUse.length > 0
      ? finalOptionsToUse.map(opt => stripTagSymbols(String(opt)))
