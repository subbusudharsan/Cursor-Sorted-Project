...
console.log(`   Has content: ${cleanRecipientSummary.length > 0 ? 'YES' : 'NO ❌'}`);
    ...
    const removeListenerName = (option: string): string => {
      ...
      if (listenerName) {
        ...
        // Replace possessive forms FIRST (before base form to avoid conflicts)
        // "Aradhya's" → "your", "Aradhya'" → "your"
        cleaned = cleaned.replace(new RegExp(`\\b${escapedName}'s\\b`, 'gi'), 'your');
        cleaned = cleaned.replace(new RegExp(`\\b${escapedName}'\\b`, 'gi'), 'your');
        
        // Replace base form with proper capitalization
        // "Aradhya said" → "You said" (if at start) or "you said" (if in middle)
        // "what Aradhya said" → "what you said"
        cleaned = cleaned.replace(new RegExp(`\\b${escapedName}\\b`, 'gi'), (match, offset, string) => {
          // Determine if this is at the start of a sentence (after punctuation/start of string)
          const beforeMatch = offset > 0 ? string.substring(Math.max(0, offset - 2), offset) : '';
          const isStartOfSentence = offset === 0 || /[.!?]\s*$/.test(beforeMatch) || /^\s*$/.test(beforeMatch);
          
          // Always replace listener's name with "you" (capitalize if at sentence start)
          // This works for both User A → User B and User B → User A
          return isStartOfSentence ? 'You' : 'you';
        });
      }
    ...
    const finalOptions = options.slice(0, Math.min(expectedCount, options.length))
      .map((opt: string) => capitalizeFirstLetter(fixPronounMistakes(removeListenerName(stripTagSymbols(opt)))));
