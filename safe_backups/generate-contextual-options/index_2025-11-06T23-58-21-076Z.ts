...
const cleanPerspective = (text: string | undefined): string => {
  ...
  const determineSentenceStartReplacement = (
    replacement: string,
    offset: number,
    source: string
  ) => {
    const prefix = source.slice(0, offset);
    const isStartOfSentence = offset === 0 || /[.!?]\s*$/.test(prefix) || prefix.trimEnd().length === 0;
    if (replacement.toLowerCase() === 'you') {
      return isStartOfSentence ? 'You' : 'you';
    }
    if (replacement.toLowerCase() === 'your') {
      return isStartOfSentence ? 'Your' : 'your';
    }
    return replacement;
  };
  ...
  if (isRecipientUserA) {
    ...
    if (listenerVariants.length > 0) {
      listenerVariants.forEach(variant => {
        ...
        // Possessive forms
        cleaned = cleaned.replace(new RegExp(`\\b${escaped}'s\\b`, 'gi'), (_, __, offset, source) =>
          determineSentenceStartReplacement('your', offset, source)
        );
        cleaned = cleaned.replace(new RegExp(`\\b${escaped}'\\b`, 'gi'), (_, __, offset, source) =>
          determineSentenceStartReplacement('your', offset, source)
        );
        if (!hasSpace) {
          cleaned = cleaned.replace(new RegExp(`@${escaped}'s\\b`, 'gi'), 'your');
          cleaned = cleaned.replace(new RegExp(`@${escaped}'\\b`, 'gi'), 'your');
        }

        // Base forms
        cleaned = cleaned.replace(new RegExp(`\\b${escaped}\\b`, 'gi'), (match, offset, source) =>
          determineSentenceStartReplacement('you', offset, source)
        );
        if (!hasSpace) {
          cleaned = cleaned.replace(new RegExp(`@${escaped}\\b`, 'gi'), 'you');
        }
      });
