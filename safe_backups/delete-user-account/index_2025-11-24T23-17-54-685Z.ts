...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      const { data: remainingIdentities, error: checkError } = await supabaseAdmin
        .from('auth.identities')
      ...
      if (checkError) {
        ...
        const remainingCount = remainingIdentities?.length || 0;
        if (remainingCount > 0) {
          const errorMsg = `Cannot delete user: ${remainingCount} row(s) still exist in auth.identities. Cleanup failed.`;
