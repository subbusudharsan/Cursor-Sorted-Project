...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      // Use raw SQL via RPC or direct Postgres queries since auth schema tables
      // may not be accessible via PostgREST .from() method
      console.log("🧹 Cleaning auth.identities...");
      const { error: identitiesError } = await supabaseAdmin.rpc('exec_sql', {
        query: `DELETE FROM auth.identities WHERE user_id = $1`,
        params: [userId]
      }).catch(async () => {
        // Fallback: try direct .from() if RPC doesn't exist
        return await supabaseAdmin.from('auth.identities')
          .delete()
          .eq('user_id', userId);
      });
      if (identitiesError) {
      ...
      const { error: refreshTokensError } = await supabaseAdmin.rpc('exec_sql', {
        query: `DELETE FROM auth.refresh_tokens WHERE user_id = $1`,
        params: [userId]
      }).catch(async () => {
        return await supabaseAdmin.from('auth.refresh_tokens')
          .delete()
          .eq('user_id', userId);
      });
      if (refreshTokensError) {
        ...
        console.log("✅ Cleaned auth.refresh_tokens");
      }
      ...
      const { error: mfaFactorsError } = await supabaseAdmin.rpc('exec_sql', {
        query: `DELETE FROM auth.mfa_factors WHERE user_id = $1`,
        params: [userId]
      }).catch(async () => {
        return await supabaseAdmin.from('auth.mfa_factors')
          .delete()
          .eq('user_id', userId);
      });
      if (mfaFactorsError) {
        ...
        console.log("✅ Cleaned auth.mfa_factors");
      }
      ...
      const { error: sessionsError } = await supabaseAdmin.rpc('exec_sql', {
        ...
        return await supabaseAdmin.from('auth.sessions')
          .delete()
          .eq('user_id', userId);
      });
      if (sessionsError) {
        ...
        console.log("✅ Cleaned auth.sessions");
      }

      // Verify cleanup succeeded by checking if any rows still exist
      console.log("🔍 Verifying cleanup succeeded...");
      const { data: remainingIdentities } = await supabaseAdmin.rpc('exec_sql', {
        query: `SELECT COUNT(*) as count FROM auth.identities WHERE user_id = $1`,
        params: [userId]
      }).catch(async () => {
        const { data } = await supabaseAdmin.from('auth.identities')
          .select('user_id', { count: 'exact', head: true })
          .eq('user_id', userId);
        return { data: data ? [{ count: data.length }] : [{ count: 0 }] };
      });
      
      if (remainingIdentities?.[0]?.count > 0) {
        console.warn(`⚠️ Warning: ${remainingIdentities[0].count} rows still exist in auth.identities`);
      }
