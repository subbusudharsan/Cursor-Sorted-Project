...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      try {
        const { error: rpcError } = await supabaseAdmin.rpc('delete_auth_user_data', {
          target_user_id: userId
        });
        
        if (rpcError) {
          console.warn("⚠️ Error calling delete_auth_user_data RPC:", rpcError);
          console.warn("⚠️ This might be expected if the function doesn't exist yet");
          console.warn("⚠️ Continuing with deleteUser attempt anyway...");
        } else {
          console.log("✅ Successfully cleaned auth schema tables");
        }
      } catch (rpcException) {
        console.warn("⚠️ Exception calling delete_auth_user_data:", rpcException);
        console.warn("⚠️ Continuing with deleteUser attempt anyway...");
      }

      // Delete the auth user account using admin API
      // Note: We've cleaned auth tables above, so deleteUser should succeed
      ...
      console.log("⚠️ Attempting deleteUser after auth schema cleanup");
