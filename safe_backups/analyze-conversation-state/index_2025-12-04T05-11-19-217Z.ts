...
function detectEmotionKeywords(message: string): string[] {
...
function calculateSentimentScore(message: string): number {
  const lowercased = message.toLowerCase();
  // Positive indicators
  const positiveWords = [
    "thank",
...
function suggestEmotionalLayer(sentimentScore: number, hasHint: boolean): string {
...
function suggestEmotionIntent(emotionKeywords: string[], hasHint: boolean): string {
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    // --- NEW: lightweight Groq reasoning to confirm pattern ---
    let groqIntent = suggestedIntent;
    let groqLayer = suggestedLayer;
    try {
...
`;
        const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: "system",
                content: "You are an emotional intelligence coach analyzing natural conversation flow."
              },
              {
                role: "user",
        ...
        if (response.ok) {
          ...
          if (jsonMatch) {
            ...
            groqIntent = parsed.intent || suggestedIntent;
            groqLayer = parsed.layer || suggestedLayer;
            console.log("🧩 Groq refined intent:", groqIntent, "layer:", groqLayer);
          }
        }
      }
    } catch (err) {
      console.warn("⚠️ Groq reasoning skipped:", err);
    }
    const result = {
      success: true,
      analysis: {
