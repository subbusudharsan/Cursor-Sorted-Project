...
function suggestEmotionalLayer(turnCount, sentimentScore, hasHint) {
  if (hasHint && sentimentScore <= 0) {
    return "🌧 Vulnerable-Healing"; // immediately switch to deeper tone
  }
  if (turnCount < 3) {
    return "💖 Warm-Empathic";
  }
  if (hasHint && turnCount >= 5 && turnCount < 10) {
    return "🌧 Vulnerable-Healing";
  }
  if (sentimentScore < -0.3) {
    return "🔥 Tense-Honest";
  }
  if (sentimentScore > 0.5 && turnCount >= 10) {
    return "🌿 Reflective-Growth";
  }
  return "💬 Direct-Balanced";
}
function suggestEmotionIntent(turnCount, emotionKeywords, hasHint) {
  // Early conversation (turns 1-3)
  if (turnCount < 3) {
    if (emotionKeywords.includes("curiosity")) return "guarded curiosity";
    return "gentle opening";
  }
  // Mid conversation (turns 4-8)
  if (turnCount < 8) {
    if (emotionKeywords.includes("curiosity")) return "seeks understanding";
    if (hasHint && emotionKeywords.includes("vulnerability")) return "reveals root hurt privately";
    if (emotionKeywords.includes("ownership")) return "ownership";
    if (emotionKeywords.includes("recognition")) return "recognition";
    return "gentle honesty";
  }
  // Late conversation (turns 9-14)
  if (turnCount < 14) {
    if (emotionKeywords.includes("forgiveness")) return "forgiveness";
    if (emotionKeywords.includes("gratitude")) return "gratitude";
    if (emotionKeywords.includes("reassurance")) return "reassurance";
    return "shares context";
  }
  // Closure phase (turns 15+)
  if (emotionKeywords.includes("gratitude")) return "mutual closure (receiver's peace)";
  return "emotional closure (initiator's peace)";
}
Deno.serve(async (req)=>{
  ...
  try {
    const { message, turnCount, hasHint, contactCategory } = await req.json();
    console.log("🔍 ANALYZER: Analyzing message...", {
    ...
    const suggestedLayer = suggestEmotionalLayer(turnCount, sentimentScore, hasHint);
    const suggestedIntent = suggestEmotionIntent(turnCount, emotionKeywords, hasHint);
...
Analyze the human tone of this message and predict which emotional phase it represents.
...
Phases: gentle opening, hesitant acknowledgment, seeks understanding, recognition, forgiveness, gratitude, closure
