// @ts-ignore - jsr imports resolved in Deno runtime
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const groqPayload = buildGroqPayload(entries, chats, weekStartIso);
    ...
    if (!response.ok) {
      console.warn("Groq nudge error", await response.text());
...
function buildGroqPayload(entries: any[], chats: any[], weekStartIso: string) {
...
function parseGroqJson(raw: string) {
  try {
    ...
    console.warn("Failed to parse Groq nudge JSON", error);
  }
  return {
