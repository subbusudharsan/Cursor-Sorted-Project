...
// 🧹 Clean @ and # tags everywhere before returning
function cleanTags(text) {
  if (!text) return text;
  return text
  // Handle @Name's and @Name’s → your
  .replace(/@\w+[’']s/g, 'your')
  // Handle @Name → you
  .replace(/@\w+\b/g, 'you')
  // Handle #Name's and #Name’s → their
  .replace(/#\w+[’']s/g, 'their')
  // Handle #Name → they
  .replace(/#\w+\b/g, 'they');
...
if (parsedResponse?.summary) parsedResponse.summary = cleanTags(parsedResponse.summary);
if (parsedResponse?.context_data?.summary) parsedResponse.context_data.summary = cleanTags(parsedResponse.context_data.summary);
if (parsedResponse?.key_points?.length) parsedResponse.key_points = parsedResponse.key_points.map((p) => cleanTags(p));
if (parsedResponse?.context_data?.key_points?.length) parsedResponse.context_data.key_points = parsedResponse.context_data.key_points.map((p) => cleanTags(p));
