/*
  ...
  1. Purpose
    ...
    - Generates a structured summary using Groq Llama AI
...
Your task is to:
1. SYNTHESIZE ALL INFORMATION from initial description AND every Q&A response AND any Additional Information provided
...
CRITICAL: The summaries MUST incorporate ALL Q&A responses AND any Additional Information section, not just the initial description. If "Additional Information" is provided, it contains important context that MUST be reflected in both the summary and thoughts sections.
...
Return ONLY valid JSON with this structure:
{
  ...
  "context_data": {
    ...
    "key_points": ["same as above"],
     ...
     "thoughts_a": "I want to approach [@contact name] calmly and explain my side so we can work through this together.",
...
Respond with ONLY the JSON structure specified in the system prompt.`
    ...
    // ✅ FIX: Check for new format (summary_a_perspective, summary_shared_neutral) OR old format (summary)
    const hasNewFormat = parsedResponse.summary_a_perspective || parsedResponse.summary_shared_neutral;
    const hasOldFormat = parsedResponse.summary;
    const hasContextData = parsedResponse.context_data;
    
    if (!hasNewFormat && !hasOldFormat) {
      console.error('❌ Invalid response format - missing both new and old summary fields');
      console.error('Response keys:', Object.keys(parsedResponse));
    ...
    if (!parsedResponse.summary_shared_neutral) {
      console.warn('⚠️ Warning: Groq did not generate summary_shared_neutral. Creating fallback.');
    ...
    if (parsedResponse.summary_a_perspective && parsedResponse.summary_shared_neutral &&
      ...
      console.warn('⚠️ WARNING: Groq returned identical summaries! This may cause issues.');
    }

    // ✅ FIX: Validate and fix "Same as above" in thoughts_a
    const thoughtsA = parsedResponse.context_data?.thoughts_a || parsedResponse.thoughts_a || '';
    if (thoughtsA && (
      thoughtsA.toLowerCase().includes('same as above') ||
      thoughtsA.toLowerCase().includes('same as thoughts above') ||
      thoughtsA.toLowerCase().includes('same as') ||
      thoughtsA.trim().length < 10 // Too short to be meaningful
    )) {
      console.warn('⚠️ WARNING: Groq returned placeholder thoughts. Generating proper fallback.');
      
      // Generate meaningful fallback thoughts based on summary and contact info
      const userB = tagged_persons?.find((p: any) => p.is_user_b);
      const userBName = userB?.name || userB?.entity_name || 'them';
      const userBTag = userB ? `@${userBName}` : 'them';
      
      // Create forward-looking thoughts based on the situation
      const fallbackThoughts = `I want to approach ${userBTag} calmly and clear any misunderstanding so we can stay comfortable with each other.`;
      
      // Update thoughts_a in both places
      if (parsedResponse.context_data) {
        parsedResponse.context_data.thoughts_a = fallbackThoughts;
        parsedResponse.context_data.thoughts = fallbackThoughts;
      } else {
        parsedResponse.context_data = {
          thoughts_a: fallbackThoughts,
          thoughts: fallbackThoughts,
        };
      }
      parsedResponse.thoughts_a = fallbackThoughts;
      parsedResponse.thoughts = fallbackThoughts;
      
      console.log('✅ Replaced placeholder thoughts with:', fallbackThoughts.substring(0, 80));
    }
