...
function detectClosureSignals(message) {
...
function calculateClosureScore(turnCount, recentMessages, emotionHistory) {
  let score = 0.0;
  // Base score from turn count (minimum 12 turns)
  if (turnCount < 12) {
    return 0.0; // Too early for closure
  } else if (turnCount >= 15) {
    score += 0.4; // Strong baseline for 15+ turns
  } else {
    score += 0.2; // Moderate baseline for 12-14 turns
  }
  ...
  // Score based on closure signal types
  const uniqueSignals = new Set(allClosureSignals);
  if (uniqueSignals.has("gratitude")) score += 0.2;
  if (uniqueSignals.has("forgiveness")) score += 0.2;
  if (uniqueSignals.has("understanding")) score += 0.15;
  if (uniqueSignals.has("relief")) score += 0.15;
  if (uniqueSignals.has("peace")) score += 0.1;
  // NEW: If new emotional hint appears, delay closure
  const recentText = recentMessages.slice(-3).join(" ").toLowerCase();
  if (recentText.includes("felt") || recentText.includes("ignored") || recentText.includes("unheard")) {
    console.log("🧠 Hint detected near closure — reducing closure readiness");
    score = Math.max(0, score - 0.3); // hold back closure
  }
  // Check for emotional progression through resolution stages
  ...
  if (hasGratitude && hasForgiveness) score += 0.2;
  else if (hasGratitude || hasForgiveness) score += 0.1;
...
Deno.serve(async (req)=>{
  ...
  try {
    const { chatId, turnCount, recentMessages, recipientId } = await req.json();
    console.log("🤔 CLOSURE EVALUATOR: Checking readiness...", {
      chatId,
      turnCount,
      messageCount: recentMessages?.length
    });
    if (!chatId || turnCount === undefined) {
      return new Response(JSON.stringify({
        error: "chatId and turnCount are required"
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    const emotionIntents = emotionHistory.map((e)=>e.emotion_intent);
    // Calculate closure score
    const closureScore = calculateClosureScore(turnCount, recentMessages || [], emotionIntents);
    // Determine if closure should be triggered
    const shouldTriggerClosure = closureScore >= 0.85;
    // Determine closure type
    let closureType = null;
    if (shouldTriggerClosure) {
      // Check if both users have expressed closure signals
      const userASignals = recentMessages.filter((_, i)=>i % 2 === 0);
      const userBSignals = recentMessages.filter((_, i)=>i % 2 === 1);
      const userAHasClosure = userASignals.some((m)=>detectClosureSignals(m).length > 0);
      const userBHasClosure = userBSignals.some((m)=>detectClosureSignals(m).length > 0);
    ...
    const result = {
      ...
      analysis: {
        ...
        recommendation: shouldTriggerClosure ? "Generate smiley-only options (🙂)" : "Continue conversation with regular options"
      }
    };
