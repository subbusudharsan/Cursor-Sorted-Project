...

{
  "turns": [
    ...
    if (isFirstTurn) {
      promptParts.push(`First turn: Include casual greeting + issue. Generate 3 turns (${firstTurnRole === "A" ? "A→B→A" : "B→A→B"}), 3 options each, natural and supportive.`);
    } else {
    ...
    const groqResponse = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: "llama-3.1-70b-instant", // ✅ Use 70B instant for better emotional analysis in pregenerated turns
        max_tokens: 2000,
        temperature: 0.7,
        messages: [
          {
