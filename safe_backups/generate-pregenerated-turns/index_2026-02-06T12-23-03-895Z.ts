...
const messagesData = (rawMessages || []).sort(
  (a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime()
    ...
    // ✅ Also check for mutual understanding signals (gratitude, forgiveness, understanding)
...
`}`;
    ...
    const shouldEnforceSmiley = (turnIndex: number, recipientId: string): boolean => {
      ...
      // The previous mapping was inverted, causing the "other user" to miss the single-smiley option.
      ...
      // ✅ NEW: Also enforce if both sides have shared AND mutual understanding detected
      ...
      return isTargetUser || closureDetected || bothSidesSharedAndUnderstood;
    };
