...
If near closure, add a smiley-only option as one of the 3.`;
    ...
    try {
      ...
      turnsData = JSON.parse(responseText);
    } catch {
      // Try extracting JSON from markdown code blocks or other formatting
      const jsonMatch = responseText.match(/\{[\s\S]*"turns"[\s\S]*\}/);
      if (jsonMatch) {
        turnsData = JSON.parse(jsonMatch[0]);
      } else {
