...
const messagesData = (rawMessages || []).sort(
  (a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime()
    ...
    // ✅ CRITICAL FIX: Only skip if ALL 3 expected turns exist
    // If only some turns exist (partial), we should regenerate to fill gaps
    // This allows initial generation of turns 0,1,2 when summary is ready
    if (hasAllExpectedTurns && existingTurns && existingTurns.length >= 3) {
      console.log(
        `✅ All 3 expected turns (${expectedTurnNumbers.join(', ')}) already exist — skipping generation to prevent flicker`
      );
      return new Response(JSON.stringify({
        success: true,
        stored: 0,
        message: 'All expected turns already exist — keeping them to avoid flicker',
        existingTurnNumbers
      }), {
        status: 200,
    ...
    // ✅ If we have partial turns (some but not all), delete and regenerate to ensure completeness
    if (existingTurnNumbers.length > 0 && !hasAllExpectedTurns) {
      console.log(`⚠️ Found ${existingTurnNumbers.length} partial turns (missing: ${missingTurns.join(', ')}), deleting and regenerating to fill gaps...`);
