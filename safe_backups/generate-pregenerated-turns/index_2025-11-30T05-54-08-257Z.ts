...
If near closure, add a smiley-only option as one of the 3.`;
    ...
    // Also clean up old used turns that are more than 10 turns behind to keep table clean
    console.log(`🧹 Cleaning up future unused pregenerated turns from turn_number ${startingTurnNumber} onwards...`);
    
    // Delete future unused turns (turns >= startingTurnNumber that are unused)
    ...
    // ✅ BONUS: Clean up old used turns that are way behind (more than 5 turns back)
    // This keeps the table clean without affecting active conversation
    if (startingTurnNumber > 5) {
      const oldTurnThreshold = startingTurnNumber - 5;
      // Delete old used turns (used_at IS NOT NULL) that are more than 5 turns behind
      const { error: oldTurnsDeleteError } = await supabase
        .from("pregenerated_turns")
        .delete()
        .eq("chat_id", chatId)
        .lt("turn_number", oldTurnThreshold) // Delete turns that are more than 5 turns behind
        .not("used_at", "is", null); // Only delete used turns (keep unused ones)
      
      if (!oldTurnsDeleteError) {
        console.log(`✅ Cleaned up old used pregenerated turns before turn_number ${oldTurnThreshold}`);
      } else {
        console.warn("⚠️ Failed to clean up old used turns (non-critical):", oldTurnsDeleteError);
      }
    }
