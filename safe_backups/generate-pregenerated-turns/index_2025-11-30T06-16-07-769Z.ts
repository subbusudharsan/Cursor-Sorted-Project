...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const messagesFromA = conversationHistory.filter(m => String(m.sender_id) === String(userAId)).length;
    const messagesFromB = conversationHistory.filter(m => String(m.sender_id) === String(userBId)).length;
