...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ✅ FIX 1: Also check for exact turn_numbers to prevent duplicates
    ...
    // ✅ FIX 1: Check for exact turn_numbers that would be generated (startingTurnNumber, startingTurnNumber+1, startingTurnNumber+2)
    const expectedTurnNumbers = [startingTurnNumber, startingTurnNumber + 1, startingTurnNumber + 2];
    const existingTurnNumbers = existingUnusedTurns?.map((t: { turn_number: number }) => t.turn_number) || [];
    const hasAllExpectedTurns = expectedTurnNumbers.every(tn => existingTurnNumbers.includes(tn));
    
    if (hasAllExpectedTurns && existingUnusedTurns && existingUnusedTurns.length >= 3) {
      console.log(`✅ Already have all 3 unused pregenerated turns for turn_numbers ${expectedTurnNumbers.join(', ')}, skipping generation`);
      return new Response(JSON.stringify({
        success: true,
        stored: 0,
    ...
    // ✅ FIX 1: If we have some but not all expected turns, delete the partial ones before regenerating
    if (existingUnusedTurns && existingUnusedTurns.length > 0 && !hasAllExpectedTurns) {
      console.log(`⚠️ Found ${existingUnusedTurns.length} partial pregenerated turns, deleting before regenerating...`);
      ...
      if (deletePartialError) {
        ...
        console.log('✅ Deleted partial pregenerated turns');
      }
    }
...
`}`;
    ...
    for (let i = 0; i < 3; i++) {
      ...
      inserts.push({
        ...
        used_at: null,
        source: 'pregenerated_turns', // ✅ Track source
