...
If near closure, add a smiley-only option as one of the 3.`;
    ...
    // ✅ FIX: Delete any existing pregenerated turns for this chat first
    // This prevents duplicate key errors and ensures fresh turns
    console.log("🧹 Cleaning up existing pregenerated turns for this chat...");
    ...
    if (deleteError) {
      console.warn("⚠️ Failed to clean up existing pregenerated turns (non-critical):", deleteError);
      // Continue anyway - the insert might still work
    } else {
      console.log("✅ Cleaned up existing pregenerated turns");
    }
    ...
    console.log('✅ Inserted turn IDs:', insertedData.map(t => ({ id: t.id, turn_number: t.turn_number, recipient_id: t.recipient_id })));
