  ...
  (a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime()
    ...
    if (hasHint) {
      ...
      if (deleteBatchError) {
        ...
        console.log(`✅ Deleted batch [${batchStart}-${batchEnd}] to regenerate with hint context`);
      }
    ...
    // ✅ CRITICAL: If hint is provided, force regeneration by deleting existing turns
    // Hint changes context, so old turns are no longer valid
    if (hasHint && existingTurnNumbers.length > 0) {
      console.log(`🔄 Hint provided - forcing regeneration by deleting ${existingTurnNumbers.length} existing turn(s) to incorporate hint context`);
      const { error: deleteForHintError } = await supabase
        .from('pregenerated_turns')
        .delete()
        .eq('chat_id', chatId)
        .gte('turn_number', startingTurnNumber)
      ...
      if (deleteForHintError) {
        console.warn('⚠️ Failed to delete turns for hint regeneration (non-critical):', deleteForHintError);
      } else {
        console.log(`✅ Deleted ${existingTurnNumbers.length} turn(s) to regenerate with hint context`);
      }
    }
