...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Check the last message to see who should get the next turn (use full history for logic)
    const lastMessage = allMessages.length > 0 ? allMessages[allMessages.length - 1] : null;
    const lastSenderId = lastMessage ? String(lastMessage.sender_id) : null;

    // Count messages from each user (use full history for turn calculation, not truncated)
    const messagesFromA = allMessages.filter((m: { sender_id: string }) => String(m.sender_id) === String(userAId)).length;
    const messagesFromB = allMessages.filter((m: { sender_id: string }) => String(m.sender_id) === String(userBId)).length;
...
**FOR USER A SPEAKING TO USER B:**
- When talking about YOURSELF (User A) → ALWAYS use "I / me / my / mine / myself"
...
**FOR USER B SPEAKING TO USER A:**
- When talking about YOURSELF (User B) → ALWAYS use "I / me / my / mine / myself"
...
- Or use pronouns from context: "she", "he", "they"
...
- Tone: calm, kind, emotionally aware, natural, human.
...

{
  "turns": [
    ...
    // ✅ TOKEN OPTIMIZATION: Simplified user prompt (keeps summary for core issue memory, removes verbose instructions)
    const isFirstTurn = allMessages.length === 0; // Check original length, not truncated
    
    // Build minimal prompt with only essential info
    const promptParts = [];
    promptParts.push(`Issue: ${summarySharedNeutral}`);
    
    if (conversationHistory.length > 0) {
      // ✅ Compact JSON format (no pretty printing)
      promptParts.push(`Recent: ${JSON.stringify(conversationHistory)}`);
    }
    
    if (hintFromB) {
      promptParts.push(`Hint: ${hintFromB}`);
    }
    
    // ✅ Short instruction based on turn type
    if (isFirstTurn) {
      promptParts.push(`First turn: Generate natural opening options that:
- Include casual greetings (hi, hey, how are you, how's it going, hope you're doing well) for ALL situations
- Match the context: happy/positive situations, long-time friends, recent arguments, or normal timing
- Then transition to the issue naturally (don't jump straight to the problem)
- Vary in style: some start with greeting, some are more direct, some are situation-aware
- Examples: "Hey, how are you? Can we talk about something?" or "Hi, hope you're doing well. Got something I wanted to discuss"
Generate 3 turns (${firstTurnRole === "A" ? "A→B→A" : "B→A→B"}), 3 options each, natural and supportive.`);
    } else {
      promptParts.push(`Continue conversation. Generate 3 turns (${firstTurnRole === "A" ? "A→B→A" : "B→A→B"}), 3 options each, natural and supportive. If near closure, include 1 smiley option.`);
    }
    ...
    const groqResponse = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: "llama-3.1-8b-instant", // ✅ Use 8B instant for consistency and cost efficiency
        max_tokens: 2000,
        temperature: 0.7,
        messages: [
          {
