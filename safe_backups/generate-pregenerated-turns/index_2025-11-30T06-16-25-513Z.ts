...
If near closure, add a smiley-only option as one of the 3.`;
    ...
    // ✅ BONUS: Clean up old used turns that are more than 10 turns behind
    // This keeps the table clean without affecting active conversation
    if (startingTurnNumber > 10) {
      const oldTurnThreshold = startingTurnNumber - 10;
      const { error: oldTurnsError } = await supabase
        .from("pregenerated_turns")
        .delete()
        .eq("chat_id", chatId)
        .lt("turn_number", oldTurnThreshold)
      ...
      if (!oldTurnsError) {
        ...
        console.warn("⚠️ Failed to clean up old used turns (non-critical):", oldTurnsError);
      }
    }
