...
const messagesData = (data || []).sort(
  (a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime()
    ...
    // NEW RULE: If ANY turns exist in this range → DO NOT regenerate.
// This prevents overwriting and flicker.
if (existingTurnNumbers.length > 0) {
  console.log(
    `⛔ Skipping generation. Turns already exist for range ${startingTurnNumber}-${startingTurnNumber + 2}.`
  );
  return new Response(JSON.stringify({
    success: true,
    stored: 0,
    message: 'Turns already exist — keeping them to avoid flicker',
    existingTurnNumbers
  }), {
    status: 200,
...
`}`;
    ...
    console.log(`🧹 Cleaning up pregenerated turns from turn_number ${startingTurnNumber} to ${startingTurnNumber + 2} before inserting new ones...`);
    ...
    if (deleteError) {
      ...
      console.log(`✅ Cleaned up pregenerated turns from turn_number ${startingTurnNumber} to ${startingTurnNumber + 2}`);
    }
